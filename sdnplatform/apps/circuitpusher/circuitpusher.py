...
"""
circuitpusher utilizes sdnplatform rest APIs to create a bidirectional circuit, 
...
Notes:
 ...
 3. The current supported command syntax format is:
    a) circuitpusher.py --controller={IP}:{rest port} --type ip --src {IP} --dst {IP} --add --name {circuit-name}
       ...
       and the user should make sure deleting the file when sdnplatform controller is restarted.
...
if args.action=='add':
    ...
    for i in range(len(parsedResult)):
        if i % 2 == 0:
            ...
            # IMPORTANT NOTE: current Sdnplatform StaticflowEntryPusher
