    ...
    def insert(self, data, return_id=False):
        ...
        if key:
            if compound_key_fields is not None:
                key_data=[]
                for field_name in compound_key_fields:
                    if field_name in data:
                        key_data.append(data[field_name])
                    elif key_data:
                        raise DatabaseError("Either all or none of the individual key fields must be specified when updating with a compound key")
                if key_data:
